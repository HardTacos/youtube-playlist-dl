...
var app = module.exports = {
    retrievePlaylist: function(playlistUrl, start_index, j) {
                ...
                if (err instanceof Error) {
                    ...
                    function displayItems(msg, i, callback) {
                        ...
                        if ((j % 10 === 0) && j < 200) {
                            ...
                            async.series([
                                function(callback) {
                                    app.downloadVideo(id, title, callback);
                                }
                            ], function(err) {
                                app.retrievePlaylist(playlistUrl, j + 1, j + 1);
                            });
                        }
                        else {
                            console.log(j + ") " + title);
    ...
    deleteVideo: function(fileName, mp4Directory, callback) {
        var removeVideo = exec('rm "' + mp4Directory + fileName + '"',
            function(error, stdout, stderr) {
                callback();
                if (error !== null) {
                    console.log('exec error: ' + error);
